...
public class WFConnection extends Object implements
	...
	private static void checkSignal(final Context context) {
		...
		if (prefs.getFlag(Pref.STATENOT_KEY) && screenstate) {
			...
			try {
				 ...
				 */
				LogService.log(ctxt, appname,
						context.getString(R.string.thanks_google)
			...
			if (signalcache == 0)
				 ...
				 */
				if (statNotifCheck())
					NotifUtil.addStatNotif(context, notifSSID,
							context.getString(R.string.network_test),
							notifSignal, true, statnotiflayout);
			}
	...
	private static boolean getHttpHeaders(final Context context)
		...
		if (httpclient == null) {
			...
			if (prefs.getFlag(Pref.LOG_KEY))
				LogService.log(context, appname,
						context.getString(R.string.instantiating_httpclient));
		}
		/*
		...
		if (prefs.getFlag(Pref.LOG_KEY) && status != HttpURLConnection.HTTP_OK) {
			LogService.log(context, appname,
					context.getString(R.string.http_status) + status);
		}
	...
	public static boolean readManagedState(final Context context,
		...
		if (PrefUtil.readNetworkPref(context,
				PrefUtil.getnetworkSSID(context, network),
	...
	private static boolean httpHostup(final Context context) {
		...
		if (prefs.getFlag(Pref.LOG_KEY))
			LogService.log(context, appname,
					context.getString(R.string.http_method));
		...
		try {
			...
			if (prefs.getFlag(Pref.LOG_KEY))
				LogService.log(context, appname,
						context.getString(R.string.httpexception));
		} catch (URISyntaxException e) {
			if (prefs.getFlag(Pref.LOG_KEY))
				LogService.log(context, appname,
						context.getString(R.string.urlexception));
		}
	...
	private static void icmpCache(final Context context) {
		...
		if (prefs.getFlag(Pref.LOG_KEY))
			LogService.log(context, appname,
					context.getString(R.string.cached_ip) + cachedIP);
	}
	...
	private static boolean icmpHostup(final Context context) {
		...
		if (prefs.getFlag(Pref.LOG_KEY))
			LogService.log(context, appname,
					context.getString(R.string.icmp_method) + cachedIP);
		...
		try {
			if (InetAddress.getByName(cachedIP).isReachable(REACHABLE)) {
				...
				if (prefs.getFlag(Pref.LOG_KEY))
					LogService.log(context, appname,
							context.getString(R.string.icmp_success));
			}
	...
	private void onNetworkConnected() {
		...
		if (!shouldManage(ctxt) && prefs.getFlag(Pref.LOG_KEY))
			LogService.log(ctxt, appname,
					ctxt.getString(R.string.not_managing_network) + getSSID());
	}
	...
	private void onWifiDisabled() {
		...
		if (prefs.getFlag(Pref.STATENOT_KEY))
			NotifUtil.addStatNotif(ctxt, NULL_SSID,
					ctxt.getString(R.string.wifi_is_disabled),
					R.drawable.signal0, true, statnotiflayout);
	...
	private static void toggleWifi() {
		...
		if (prefs.getFlag(Pref.STATENOT_KEY))
			NotifUtil.addStatNotif(ctxt, NULL_SSID,
					ctxt.getString(R.string.toggling_wifi), R.drawable.signal0,
					true, statnotiflayout);
	}
